...
from cvat.apps.engine.models import AttributeType, ShapeType, Task
...
from ..engine.log import slogger
...
class CvatTaskDataExtractor(datumaro.SourceExtractor):
    def __init__(self, task_data, include_images=False):
        ...
        print("--------------")
        print("task_data_meta",task_data.meta)
        no_of_frames =int(task_data.meta['task']['size'])
        print("task_data_size--->",no_of_frames)

        print("--------------")

        task_id = 0
        source = ""
        for k,v in task_data.meta.items():
            if k == "task" :
            ...
            #source=data['client_files'][0]

        #file_path='9.txt'.format(task_id)
        ...
        print("-----------------------job id----->>>>>>>>>>>>>>>>>>>>>>>>>")
        print("--------ggggggggggggggggggggggggggggggggggg------",job_id)
        slogger.glob.info("--------ggggggggggggggggggggggggggggggggggg------",db_data.id)



        def CartionFromLatLongAlt(WGS84_Ref_Altitude, WGS84_Ref_Latitude, WGS84_Ref_Longitude):
            print("---",WGS84_Ref_Altitude, WGS84_Ref_Latitude, WGS84_Ref_Longitude)

            deg2rad = 0.017453229
            rad2deg = 57.2957795
            lat_rad = WGS84_Ref_Latitude * deg2rad
            long_rad = WGS84_Ref_Longitude * deg2rad
            alt_rad = WGS84_Ref_Altitude * deg2rad

            print("degrer to rad--->",lat_rad,"--",long_rad)

            WGS84_EARTH_RADIUS = 6378137.0;  #Major axis
            WGS84_Radius = 6356752.3142;     #Minor axis

            WGS84_ECC_FLT = (WGS84_EARTH_RADIUS - WGS84_Radius)/WGS84_EARTH_RADIUS;  #Eccentricity
            radsqr = WGS84_EARTH_RADIUS*WGS84_EARTH_RADIUS
            fltsqr = WGS84_ECC_FLT*(2-WGS84_ECC_FLT)
            s = math.sin(lat_rad)
            N = WGS84_EARTH_RADIUS/(math.sqrt(1- fltsqr*s*s))

            #WGS to ECEF conversion
            RefEcefX = (N+WGS84_Ref_Altitude)*math.cos(lat_rad)*math.cos(long_rad)
            RefEcefY = (N+WGS84_Ref_Altitude)*math.cos(lat_rad)*math.sin(long_rad)
            RefEcefZ = (N*(1-WGS84_ECC_FLT*WGS84_ECC_FLT)+WGS84_Ref_Altitude)*math.sin(lat_rad)

            print("---RefEcefX---RefEcefY---RefEcefZ--->",RefEcefX,RefEcefY,RefEcefZ)

            return RefEcefX,RefEcefY,RefEcefZ
        ...
        def dm_to_sd(dm):
            ...
            return float("{0:.5f}".format(float(d) + float(m) / 60))



        def getAltLatLng(altString,latString,lngString):
            alt = float(dm_to_sd('{}'.format(altString)))
            lat = float(dm_to_sd('{}'.format(latString)))
            lng = float(dm_to_sd('{}'.format(lngString)))
        ...
        #filePath = "/mnt/c/Users/Hi/sam_nava/cvat/cvat/apps/dataset_manager/9.csv"
        #file path for docker
        filePath = '/home/django/data/data/{}/{}.csv'.format(job_id,job_id)




        def get_alt_long_lat():
            read_path = filePath
            list_of_dict = []
            list_of_cordinates = []
            with open(read_path, 'r') as file:
                    ...
                    for row in reader:
                        ...
                        if status == 'A':
                            ...
                            lat = dm_to_sd(0)
                            lng = dm_to_sd(0)
                        list_of_dict.append(getGPS(alt,lat,lng))
                        x,y,z = CartionFromLatLongAlt(alt,lat,lng)
                        list_of_cordinates.append(getAxis(x,y,z))

            return list_of_dict,list_of_cordinates

        tuple_of_gps, tuple_of_axis = get_alt_long_lat()
        list_of_gps = list(tuple_of_gps)
        list_of_axis = list(tuple_of_axis)
        no_of_gps_data = len(list_of_gps)

        print("----list of gps-->",list_of_gps,"---->list of axis-->",list_of_axis)
        print("----no of gps--->",no_of_gps_data,"----size of task_data-->")

        gps_changing_no = round( no_of_frames /no_of_gps_data )

        print("---evry gps data shoud be present in ----",gps_changing_no,"---frmaes")
        ...
        for frame_data in task_data.group_by_frame(include_empty=True):
            ...
            if counter == 1:
                ...
                if index+1 > no_of_gps_data:
                    gps_data_value = list_of_gps[no_of_gps_data-1]
                    gps_axis_value = list_of_axis[no_of_gps_data-1]
                else:
                    gps_data_value = list_of_gps[index]
                    gps_axis_value = list_of_axis[index]

            print("--------frame id---------",frame_data.idx)

            if ( counter % gps_changing_no ) == 0:
                counter = 1
                index += 1
                print("--after this frame again fetching gps data--->",counter)
            else :
                 counter += 1

            #print("frame_data--->",frame_data)
            print("gps_data--->",gps_data_value)
            print("gps axis--->",gps_axis_value)
            ...
            #print("dm_iamge==",dm_image)

            dm_anno = self._read_cvat_anno(frame_data, task_data, gps_data_value, gps_axis_value)
            dm_item = datumaro.DatasetItem(id=osp.splitext(frame_data.name)[0],
                annotations=dm_anno, image=dm_image,
                attributes={'frame': frame_data.frame})
            print("dm_items---->",dm_item)
            dm_items.append(dm_item)
            #print("dm_items----",dm_items)
    ...
    def _load_categories(cvat_anno):
        ...
        print("label_cat",label_categories)
        categories[datumaro.AnnotationType.label] = label_categories

        return categories

    def _read_cvat_anno(self, cvat_frame_anno, task_data, gps_data, gps_axis):
        item_anno = []

        print("frame anno---->",cvat_frame_anno)
        print("gps_data---->",gps_data)
        print("gps axis---->",gps_axis)
        ...
        print("label attributes---->",label_attrs)

        def convert_attrs(label, cvat_attrs):
            print("convert attr called--------------->>>>>>")
            print("--------->",label,"----->",cvat_attrs)
            ...
            for _, a_desc in label_attrs[label]:
                ...
                print("----name---->",a_name,"---value---->",a_value)
                try:
                    ...
                    raise Exception(
                        ...
                        (a_name, a_value, e))
            print("dm_attr",dm_attr)
            return dm_attr

        #file1 = open('/mnt/c/Users/Hi/sam_nava/cvat/cvat/apps/dataset_manager/9.csv', 'r')
        ...
        for tag_obj in cvat_frame_anno.tags:
            ...
            print("item anno=======>>>>",item_anno)

        print("size of labed shape----->",len(cvat_frame_anno.labeled_shapes))
        ...
        for shape_obj in cvat_frame_anno.labeled_shapes:
            ...
            if shape_obj != "None":
                ...
                print("------shape_obj.label----",shape_obj.label)
                if str(shape_obj.label) == "EgoVehicle":
                    print("yes")
                    anno_attr['latitude'] = gps_data[1]
                    anno_attr['longitude'] = gps_data[2]

                anno_attr['occluded'] = shape_obj.occluded


                #anno_gps['latitude']=gps_data[0]
                #anno_gps['longitude'] = gps_data[1]
                ...
                anno_points = list(shape_obj.points) + list(gps_axis)
                print("-------after--->shapeobj.points------>",anno_points)
                print("---------------------------->shape obj type------->",shape_obj.type,"--------------======= shape typr------>>>",ShapeType.POINTS)
                if shape_obj.type == ShapeType.POINTS:
                    ...
                    x0, y0, x1, y1 ,x ,y ,z = anno_points
                    anno = datumaro.Bbox(x0, y0, x1 - x0, y1 - y0, x, y, z,
                        label=anno_label, attributes=anno_attr, group=anno_group,
                        z_order=shape_obj.z_order)
                    print("---if--->",anno)
